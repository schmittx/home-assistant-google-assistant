...
class GoogleEntity:
    ...
    def sync_serialize(self, agent_user_id, instance_uuid):
        ...
        elif device_entry.manufacturer:
            device_info["manufacturer"] = device_entry.manufacturer
        else:
            device_info["manufacturer"] = "Home Assistant"

        if model := entity_config.get(CONF_MODEL):
            device_info["model"] = model
        elif device_entry.model:
            device_info["model"] = device_entry.model

        if sw_version := entity_config.get(CONF_SOFTWARE_VERSION):
            device_info["swVersion"] = sw_version
        elif device_entry.sw_version:
            device_info["swVersion"] = device_entry.sw_version

        if hw_version := entity_config.get(CONF_HARDWARE_VERSION):
           device_info["hwVersion"] = hw_version
        elif device_entry.hw_version:
            device_info["hwVersion"] = device_entry.hw_version
        # Custom Component <<
